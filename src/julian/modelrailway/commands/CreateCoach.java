...
private static final String REGEX = "create coach (passenger|freight|special) (\\d+) (true|false) (true|false)"; 
    ...
    public void execute ( String command) {
        try {
            model.getRollStock().createCoach(getMatcher(command).group(1), Integer.parseInt(getMatcher(command).group(2)), "true".contentEquals(getMatcher(command).group(3)), "true".contentEquals(getMatcher(command).group(4)));
